...
- `POST /api/webhooks/stripe`: the single canonical Stripe endpoint. Verifies signatures against the raw body, enforces `STRIPE_ENVIRONMENT`/`livemode` separation (fails closed), persists and deduplicates events, fulfills paid orders idempotently, releases store credit on failed/expired sessions, revokes on full refunds and lost disputes, and records partial refunds / won disputes in `payment_reviews` for manual review instead of acting automatically.
