...
test("gift card products are detected by category and slug", () => {
  ...
  assert.equal(isGiftCardProduct({ slug: "realvip-3m", category: "supporter" }), false)
})

test("a direct API request cannot buy a gift card in production", () => {
  const rejection = rejectDisabledProducts([{ slug: "realvip-3m", category: "supporter" }, { slug: "gift-card-25", category: "gift_cards" }], {})
  assert.ok(rejection)
  assert.equal(rejection?.code, "gift_cards_disabled")
})

test("normal products are unaffected by the gift card gate", () => {
  assert.equal(rejectDisabledProducts([{ slug: "realvip-3m", category: "supporter" }], {}), null)
})
...
test("the same cart produces a stable fingerprint regardless of item order", () => {
  const a = buildCartFingerprint({
    ...baseCart,
    items: [
      { productId: "realvip-3m", quantity: 1 },
      { productId: "realpets-3m", quantity: 1 }
    ]
  })
...
test("fingerprint changes with cart, account, delivery target, or credit choice", () => {
  ...
  assert.notEqual(base, buildCartFingerprint({ ...baseCart, items: [{ productId: "realpets-3m", quantity: 1 }] }))
  assert.notEqual(base, buildCartFingerprint({ ...baseCart, items: [{ productId: "realvip-3m", quantity: 2 }] }))
...
test("an attempt reused with a MODIFIED cart is rejected", () => {
  ...
  const modified = buildCartFingerprint({ ...baseCart, items: [{ productId: "realpets-3m", quantity: 1 }] })
